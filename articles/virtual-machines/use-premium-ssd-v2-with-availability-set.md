...
Premium SSD v2(Pv2) managed disks are supported with Virtual Machine(VM)s in availability sets (AvSets) to enhance the high availability and resilience of your applications. When VMs using Pv2 are part of an Availability Set(AvSet), the platform ensures that their disks are automatically distributed across multiple storage Fault Domains (FDs). This distribution minimizes the risk of a single point of failure. 
...
AvSets have fault isolation for many possible failures, to minimize single points of failure and to offer high availability.  If there's a failure in one storage FD, only the VM instances with Pv2 disks on that specific FD is affected. The other VM instances, whose disks are placed on separate Fault Domains, remain unaffected and continue to operate normally. AvSets are susceptible to certain shared infrastructure failures, like datacenter network failures, physical hardware failures or power interruptions which can affect multiple fault domains. 

In, a production scenario with three VMs deployed in an AvSets using Pv2 with Fault Domain 3, VMs and disks are distributed across multiple fault domains. When a storage fault domain fails, only the VMs in that domain are affected. The other VMs continue running, which helps maintain application availability. 

When a Pv2 disk is located in one fault domain and is attached to a VM in another, the system triggers a background copy. This process moves the disk to match the VM’s fault domain, helping ensure consistent alignment between compute and storage for better reliability and availability. 
...
* Attach a new Pv2 disk to existing VMs in an availability set
 
```azurecli-interactive
az vm disk attach -g MyResourceGroupName --vm-name MyVMname --name MyDiskName --new --sku PremiumV2_LRS --size-gb MySize
```
 
* Attach existing Pv2 disk to existing VMs in an Availability Set:
