...
| `InVMAccessControlProfileNotFound` | The gallery `InVMAccessControlProfile` '{0}' isn't available. Verify that the `InVMAccessControlProfileReferenceId` passed in is correct. | Check that the profile exists and is replicated in the regions where VM/VMSS exists. | 
