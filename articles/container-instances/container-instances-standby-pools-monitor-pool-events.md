...
To send information to the Log Analytics workspace configured, set up diagnostic settings for your standby pool resource. After configuring the diagnostic settings, it takes about 30 minutes before any logs will begin showing up in the log analytics workspace. Events that occureed before configuring the log analystics workspace will not be included. 
